...
# AppStore的git仓库地址（必选）
git_repo_url=(
  "https://mirror.ghproxy.com/https://github.com/okxlin/appstore"
  "https://mirror.ghproxy.com/https://github.com/qwenode/1panel-appstore"
...
function Cycle_git_repos() {
  declare -a repo_urls=("$@") # 将传递进来的参数当作数组存储
  for url in "${repo_urls[@]}"; do
    ...
    if [ -d "$repo_dir" ]; then
      ...
      clone_git_repo "$url" "$git_username" "$git_password" "$git_access_token" "$git_branch" "$git_depth" 2>>"$log_file"
    fi
    ...
    logs "$repo_username/$repo_projectname :1panel本地app同步成功，enjoy it!"
    logs ""
  done
}
